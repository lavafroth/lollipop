...
impl KeyState {
    fn transition(&mut self, time: SystemTime) {
        *self = match self {
...
#[derive(Debug)]
pub struct InternalState(BTreeMap<KeyCode, KeyState>);
...
impl InternalState {
    ...
    fn led_state(&self) -> i32 {
        if self.0.values().any(|v| v.pressed_state() > 0) {
            i32::MAX
        } else {
            0
        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut state = InternalState(BTreeMap::default());
    ...
    for key in [
        ...
        state.0.insert(key, KeyState::None);
    }
